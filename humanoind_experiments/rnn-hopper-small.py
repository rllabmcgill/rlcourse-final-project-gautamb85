...
project_path='/misc/scratch03/reco/bhattaga/data/i-vectors/dnn_projects/RL/cloning-priority-humanoid-sna/'
...
l_encf = lasagne.layers.GRULayer(l_in, num_units=300, name='GRUEncoder', 
    mask_input=None, gradient_steps=-1,grad_clipping=100,only_return_final=False)
l_encb = lasagne.layers.GRULayer(l_in, num_units=300, name='GRUEncoderB', 
...
network['ff1'] = DenseLayer(network['input'],200,nonlinearity=lasagne.nonlinearities.tanh)

network['ff2'] = DenseLayer(network['ff1'],200,nonlinearity=lasagne.nonlinearities.tanh)
...
while 'true':
    ...
    for data in demo_train_stream.get_epoch_iterator():
        s_data,a_data = data
        a_data = np.reshape(a_data,(a_data.shape[0],a_data.shape[1],17))
        c_data = np.concatenate([s_data,a_data],axis=2)

        bc_sdata = np.reshape(s_data,(-1,376))
        bc_adata = np.reshape(a_data,(-1,17))

        for batch in iterate_minibatches(bc_sdata, bc_adata, 64, shuffle=True):        
          inputs,targets = batch

          demo = np.repeat(c_data,inputs.shape[0],axis=0)
