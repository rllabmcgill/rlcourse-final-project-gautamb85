...
project_path='/misc/scratch03/reco/bhattaga/data/i-vectors/dnn_projects/RL/cloning-hopper-v3/'
...
sdemo = np.load('/misc/data15/reco/bhattgau/Rnn/code/Code_/RL/homework/hw1/latest-data/humanoid/expert-states-hopper-nonoise.npy')
sdemo = np.asarray(sdemo, dtype='float32')

strain = sdemo[:100,:,:]
states_train = np.reshape(strain,(-1,11))
svalid = sdemo[100:,:,:]
states_valid = np.reshape(svalid,(-1,11))


ademo = np.load('/misc/data15/reco/bhattgau/Rnn/code/Code_/RL/homework/hw1/latest-data/humanoid/expert-actions-hopper-nonoise.npy')
ademo = np.asarray(ademo,dtype='float32')
ademo = np.reshape(ademo,(ademo.shape[0],500,3))
drt = range(ademo.shape[0])

atrain = ademo[:100,:,:]
actions_train = np.reshape(atrain,(-1,3))
avalid = ademo[100:,:,:]
actions_valid = np.reshape(avalid,(-1,3))


train_set = IndexableDataset(indexables=OrderedDict([('states',states_train),('actions',actions_train)]), axis_labels=OrderedDict([('states',('batch','tsteps','feat_dim')),('actions',('batch','feat_dim'))]))
valid_set = IndexableDataset(indexables=OrderedDict([('states',states_valid),('actions',actions_valid)]), axis_labels=OrderedDict([('states',('batch','tsteps','feat_dim')),('actions',('batch','feat_dim'))]))
...
while 'true':
    ...
    for data in train_stream.get_epoch_iterator():
        ...
        t_data,t_labs = data
        #select a random demonstration and duplicate it
        tsel = random.sample(drt,1)
        tsel = tsel[0]
        demo = ademo[tsel,:,:]
        demo = demo + 0.001*np.random.standard_normal(size=(500,3))
        demo = demo[None,:,:]
        demo = np.asarray(demo,dtype='float32')

        demo_mbatch = np.repeat(demo,t_data.shape[0],axis=0)

        terr = train_func(t_data,demo_mbatch, t_labs)
        train_err += terr[0]
        train_batches += 1
    ...
    for data in valid_stream.get_epoch_iterator():
        ...
        demo_mbatch = np.repeat(demo,v_data.shape[0],axis=0)

        err = val_func(v_data, demo_mbatch,v_labs)
        val_err += err[0]
        val_batches += 1
