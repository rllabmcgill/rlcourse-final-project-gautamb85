...
project_path='/misc/scratch03/reco/bhattaga/data/i-vectors/dnn_projects/RL/hopper-random-v1/'
...
states_train = np.reshape(states_train,(-1,11))
states_valid = np.reshape(states_valid,(-1,11))
...
atrain = np.reshape(atrain,(atrain.shape[0],atrain.shape[1],3))
actions_train = atrain[:100,:,:]
actions_valid = atrain[100:,:,:]

act_train = np.copy(actions_train)
act_valid = np.copy(actions_valid)
act_train = np.reshape(act_train,(-1,3))
act_valid = np.reshape(act_valid,(-1,3))
...
while 'true':
    ...
    dscheme1 = SequentialScheme(examples=valid_demos.num_examples, batch_size=16)
    ...
    for data in demo_train_stream.get_epoch_iterator():
        ...
        sdemos = np.reshape(sdemos,(s_data.shape[0],500,11))

        ademos = [actions_train[i,:,:] for i in dinds]
        ademos = np.asarray(ademos,dtype='float32')
        ademos = np.reshape(ademos,(s_data.shape[0],500,3))

        demo = np.concatenate([sdemos,ademos],axis=2)
        print(demo.shape)

        terr = train_func(s_data,ademos,a_data)
        train_err += terr[0]
        train_batches += 1
    ...
    for data in demo_valid_stream.get_epoch_iterator():
      ...
      verr = train_func(s_data,ademos,a_data)
      val_err += verr[0]
      val_batches += 1
