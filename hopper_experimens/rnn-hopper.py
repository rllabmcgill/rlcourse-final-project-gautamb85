...
project_path='/misc/scratch03/reco/bhattaga/data/i-vectors/dnn_projects/RL/cloning-priority-hopper-500-na3/'
...
network['input'] = InputLayer(shape=(None,11), input_var = X)
...
dtrain = np.load('/misc/data15/reco/bhattgau/Rnn/code/Code_/RL/homework/hw1/latest-data/expert-states-hopper-500.npy')
dtrain = np.asarray(dtrain, dtype='float32')

demo_train = dtrain[:100,:,:]
demo_valid = dtrain[100:,:,:]

atrain = np.load('/misc/data15/reco/bhattgau/Rnn/code/Code_/RL/homework/hw1/latest-data/expert-actions-hopper-500.npy')
atrain = np.asarray(atrain,dtype='float32')
atrain = np.reshape(atrain,(120,500,3))

actions_train = atrain[:100,:,:]
actions_valid = atrain[100:,:,:]

train_demos = IndexableDataset(indexables=OrderedDict([('dmonstrations',demo_train),('action-demo',actions_train)]), axis_labels=OrderedDict([('demonstrations',('batch','tsteps','feat_dim')),('action-demos',('batch','tsteps','f_dim'))]))
valid_demos = IndexableDataset(indexables=OrderedDict([('demonstrations',demo_valid),('action-demo',actions_valid)]), axis_labels=OrderedDict([('demonstrations',('batch','tsteps','feat_dim')),('action-demos',('batch','tsteps','f_dim'))]))
...
while 'true':
    ...
    dscheme = ShuffledScheme(examples=train_demos.num_examples, batch_size=1)
    dscheme1 = SequentialScheme(examples=valid_demos.num_examples, batch_size=1)
    ...
    for data in demo_train_stream.get_epoch_iterator():
        s_data,a_data = data
        a_data = np.reshape(a_data,(a_data.shape[0],a_data.shape[1],3))
        c_data = np.concatenate([s_data,a_data],axis=2)

        bc_sdata = np.reshape(s_data,(-1,11))
        bc_adata = np.reshape(a_data,(-1,3))

        for batch in iterate_minibatches(bc_sdata, bc_adata, 64, shuffle=True):        
          inputs,targets = batch

          demo = np.repeat(a_data,inputs.shape[0],axis=0)
          terr = train_func(inputs,demo,targets)
          train_err += terr[0]
          train_batches += 1
    ...
    for data in demo_valid_stream.get_epoch_iterator():
      ...
      for batch in iterate_minibatches(bc_sdata, bc_adata, 64, shuffle=True):        
        ...
        demo = np.repeat(a_data,inputs.shape[0],axis=0)
        verr = train_func(inputs,demo,targets)
        val_err += verr[0]
        val_batches += 1
